...
static uint8_t servosInitialized = FALSE;	// the boot init already ran, see FAC_servo_init
...
static void FAC_servo_apply_new_freq(void);
 ...
 * @IMPORTANT		Two things in here disturb a servo that is already being driven, and
 * 					FAC_app_init_all_modules() re-runs this on every apply from the fac tool:
 * 					FAC_servo_apply_new_freq() goes through HAL_TIM_Base_Init(), which generates an
 * 					update event on tim3 and restarts the counter in the middle of the pulse being
 * 					produced (the servo sees a truncated or a double length pulse and twitches), and
 * 					the two disables below drop the signal. Both are therefore limited to what needs
 * 					them: the frequency is only reapplied when it really changed, and the disables
 * 					only run at boot. On a re-init the mapper decides enable and position anyway, on
 * 					the next loop pass
 */
void FAC_servo_init(void) {
	uint16_t freq = FAC_settings_GET_value(FAC_SETTINGS_CODE_SERVOS_FREQ);
	uint8_t firstInit = !servosInitialized;
	...
	uint8_t freqChanged = firstInit || (freq != FAC_servo_GET_servo_freq(1));
	FAC_servo_SET_freq(freq);
	if (freqChanged)
