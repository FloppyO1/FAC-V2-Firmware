...
- **CLI build**: `make -C Debug all` (needs `arm-none-eabi-gcc` from *GNU Tools for STM32 13.3.rel1* and `make` on PATH — neither is on PATH by default).
- **Compile-checking a single file without the IDE**: the toolchain ships inside STM32CubeIDE, at `<CubeIDE>/plugins/com.st.stm32cube.ide.mcu.externaltools.gnu-tools-for-stm32.13.3.rel1.*/tools/bin`. Prepending that to `PATH` lets you run `arm-none-eabi-gcc -c` on one `.c` with the flags below plus `-ICore/Inc -IDrivers/... -IUSB_DEVICE/... -IMiddlewares/...`, which needs **none** of the generated `Debug/` makefiles. Worth doing on every file you touch before handing work over — it catches typos and `-Wall` warnings. It is not a link, so it proves nothing about the final binary; full builds still go through the IDE.
...
- **`fac_imu` / `Libraries/LSM6DS3`**: accel+gyro over I2C1, **integer only — no floats left in this module**. **Always check `FAC_IMU_GET_status() != HAL_ERROR` before trusting IMU data** — init failure is signalled by 20 LED blinks at boot and is non-fatal, and a *runtime* read failure sets the same status while leaving the last values in place, so the status is what tells fresh data from stale.
  - The read model is **`FAC_IMU_update()` then `GET`**: one I2C transaction fetches all six axes (the gyro and accel output registers are contiguous, `0x22`…`0x2D`), and every accessor afterwards is a free field read. It is safe to call unconditionally — it no-ops before init completes, no-ops on a second call in the same millisecond (the sensor only produces a sample every 2.4 ms at 416 Hz anyway, so consumers of one loop pass share a transaction), and after a failure retries only once a second so a dead sensor cannot spend a blocking timeout per pass.
  - `BDU` and `IF_INC` in `CTRL3_C` are **load-bearing**: `IF_INC` is what makes a burst read walk consecutive registers, `BDU` is what stops a multi-byte read from straddling two samples. Both `WHO_AM_I` values are accepted — `0x69` (LSM6DS3) and `0x6A` (LSM6DS3TR-C), which share this register map.
  - Units: the driver stores **raw counts and converts nothing**; `fac_imu` owns the conversion via `fac_math.h` group 2. `_mdps` (milli-deg/s) costs **no division** because the sensitivity is a whole 70 mdps/count — that is why it is milli and not whole dps; `_mg` costs one. A mix should generally take `_raw` and `FAC_math_from_range` instead.
  - **Gyro full scale is ±2000 dps ≈ 333 RPM** — enough for stabilisation or self-righting, but a melty brain saturates it well below working speed; the usual alternative is centripetal acceleration (`ω = √(a/r)`), bounded in turn by the ±16 g accelerometer and by the IMU's distance from the spin axis.
...
Group 2 (raw fixed point) has since acquired the consumer it was written for: `fac_imu.c` and `Libraries/LSM6DS3.c` are integer too — **zero soft-float calls**, zero divisions in the driver, one division per `_mg` accessor and none per `_mdps`. The IMU was the last float island in the firmware; what remains is `map_float` in `fac_app.c`, outside every hot path.
...
**[docs/README_MIX_EDITOR_API.md](docs/README_MIX_EDITOR_API.md) is the API reference for this tool** — the full contract the generated code has to respect: execution model (the chain runs *only* in `FAC_STATE_NORMAL`, and only for a mix/function a device actually links to), the value contract and the exactness rules a simulator must mirror, the immutable boilerplate, every `fac_math` primitive with its exact semantics / division count / range limit, the sensor API (IMU, battery, ADC, receiver) with its failure modes, state and time for stateful blocks, the hard rules for generated code, and both registration paths. Keep it in sync when a signature in `fac_math.h`, `fac_mixes.h`, `fac_functions.h` or a sensor header changes. Two things it documents that are not obvious from anywhere else: **the main loop never calls `FAC_IMU_update()`** (only telemetry does, so a mix reading the IMU must call it itself or it gets arbitrarily stale data), and **the five mapper settings have a hardcoded `max` of `200+10`**, so a 12th special function would register and run but be unlinkable to any device with nothing warning about it.
...
The list found during the API documentation pass is **closed**: #1–#4, #6, #7 and #9–#29 are fixed, #5 was withdrawn as a misdiagnosis. Full descriptions, with the mechanism and the fix applied to each, in [README_API.md § 11](docs/README_API.md#11-known-issues). **Numbering is stable** — a closed issue keeps its number and moves to [§ 11.1 Fixed](docs/README_API.md#111-fixed) or [§ 11.2 Withdrawn](docs/README_API.md#112-withdrawn-not-bugs); never renumber the others.

**#22–#29 came out of the IMU pass.** The two worth remembering as patterns rather than as fixed bugs: #22, where `avg = (avg + sample) / 2` inside a loop *looks* like an average of N samples but is an exponential filter whose noise floor is that of three samples regardless of N — a whole second of boot buying nothing; and #24, where a status field overwritten by a later init step silently **disarmed the `WHO_AM_I` check upstream of it**, so identification could fail with no visible consequence. Both were invisible from the outside: the firmware behaved plausibly in each case.

**#19–#21 were found from a live hardware report** (motors and servos kicking hard enough to brown out the supply on every FAC Tool "apply"), not from code reading — a reminder that hardware symptoms are worth chasing even when the code looks locally correct. #19 is the module re-init trap described above under *A module `init()` is therefore called on a live...*; #20 is the missing watchdog refresh described under *Watchdog*; #21 is the mapper's unmapped-motor gap, now closed by the `else` branches in `FAC_mapper_apply_to_devices()` mirroring what it already did for servos.
...
- `DEBUG_UTILS` → `FUNCTION_CLONOMETER` — compiles `fac_debug_utils` (see [README_API.md § 8.19](docs/README_API.md#819-fac_debug_utils--on-target-profiling)), a TIM6 stopwatch at 1 µs/tick that returns elapsed microseconds, or `0` if the measured block exceeded 65.5 ms. This is the tool the whole performance pass was driven by — **measure before optimizing, the two wrong guesses in this file's history were both corrected by it**.
  - The measurement points are selected by `CRONOMETER_ENTIRE_FAC_APP` / `CRONOMETER_FAC_APP` / `CRONOMETER_FAC_MAPPER`. **Exactly one at a time**: there is a single TIM6 counter, so a measurement started inside another resets it and both results are wrong. A `#if` in `config.h` *counts* the enabled switches and raises `#error` on more than one — it must stay a count, an `&&` chain would only fire when all of them are on and would miss the real hazard of two.
  - Results land in `static volatile uint16_t` file-scope variables to be watched live from the debugger. `volatile` is not about concurrency: without it `-O3` deletes a variable that is only ever written.
